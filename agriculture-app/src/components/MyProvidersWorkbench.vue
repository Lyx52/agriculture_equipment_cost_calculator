<script setup lang="ts">
  ...
  const addEmployee = async () => {
    await adjustmentStore.addAdjustmentAsync({
      id: uuid(),
      adjustment_type_code: Codifiers.CustomAdjustmentsOperations,
      value: 25,
      name: 'Ārējais pakalpojumu sniedzējs',
...
<template>
  <div class="d-flex flex-column mb-3">
    <div class="d-flex flex-row">
      <h5 class="card-title">Ārējie pakalpojumu sniedzēji</h5>
    </div>
    <BTableSimple hover no-border-collapse outlined responsive caption-top class="w-100 mb-0 overflow-y-auto common-table-style">
      ...
      <BTbody v-else>
        <BTr v-for="row in adjustmentStore.customOperationAdjustments" v-bind:key="row.id">
      ...
      <BTfoot class="position-sticky bottom-0 in-front">
        <BTr>
          <BTd colspan="7">
            <BButton variant="success" size="sm" @click="addEmployee">Pievienot pakalpojumu sniedzēju</BButton>
          </BTd>
        </BTr>
      </BTfoot>
    </BTableSimple>
