<template>
    <div class="dropdown" :id="props.id ?? `select-equipment-dropdown-${uuid()}`">
        <BOverlay
        ...
        >
            <BOverlay
                ...
                spinner-small
            >
                <li v-if="showExisting"><span class="ms-3 text-center fw-bold">Esošā tehnika</span></li>
                <li v-if="showExisting" v-for="item in equipmentCollectionStore.getEquipmentByTypeCategoryAndOperation(props.equipmentTypeCategory)" class="item-height">
                    <a class="dropdown-item cursor-pointer" @click="onClickEquipment(item)">
                        {{ item.fullEquipmentName }}
                    </a>
                </li>
                <BDropdownDivider v-if="showExisting" />
                <li v-if="showExisting"><span class="ms-3 text-center fw-bold">Pievienot jaunu tehniku</span></li>
...
import {getEquipmentSubTypes, type OperationType} from "@/stores/constants/OperationTypes";
...
const filterStore = useQuickEquipmentFilterStore(props.filterStoreId ?? uuid());
watch(() => props.operationType, async (newOperation, oldOperation) => {
    model.value = undefined;
    filterStore.$reset();
    await onChangeOperation(props.equipmentTypeCategory);
});
...
const onChangeOperation = async (equipmentTypeCategory: EquipmentTypeCategory) => {
    filterStore.$patch({
        filteredEquipmentTypes: getEquipmentTypesByCategory(equipmentTypeCategory),
        filteredEquipmentSubTypes: getEquipmentSubTypes(equipmentTypeCategory)
    });
