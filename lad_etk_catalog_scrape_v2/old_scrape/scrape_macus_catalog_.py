...
def get_model(listing: dict) -> dict:
    ...
    return info[0]
def get_tractor_catalog(page: int, base_url) -> dict:
    result = {}
    res = requests.get(f"{base_url}/+/{page},100,relevance,search.html")
...
def save_listings(data):
     with open("macus_catalog_bulb_harvester.json", "w") as fp:
        data = json.dump(data, fp, indent=4)
        fp.close()
if os.path.exists('macus_catalog_bulb_harvester.json'):
    with open("macus_catalog_bulb_harvester.json", "r") as fp:
        data = json.load(fp)
        fp.close()
else:
    data = {}
...
base = 'https://www.mascus.com/+/categorypath%3dagriculture%252fagriharvesters%252fbulbharvesters'
max_page = 1
...
if 'pages' in data:
    ...
    for page in range(1, max_page + 1):
        if page not in [int(p) for p in data['pages'].keys()]:
            ...
            listings = get_tractor_catalog(page, base)
