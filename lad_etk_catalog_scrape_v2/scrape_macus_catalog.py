...
def save_listings(data):
     with open("data/macus/macus_catalog_tractor.json", "w") as fp:
        data = json.dump(data, fp, indent=4)
        fp.close()
if os.path.exists('data/macus/macus_catalog_tractor.json'):
    with open("data/macus/macus_catalog_tractor.json", "r") as fp:
        data = json.load(fp)
        fp.close()
else:
    data = {}
