...
for page in range(1, max_pages + 1):
    table = None
    if not os.path.exists(f"data/lad_catalog/raw/pages/page_{page}_table.html"):
        table = get_lad_catalog_table(page)
        with open(f"data/lad_catalog/raw/pages/page_{page}_table.html", "w", encoding="UTF-8") as fp:
            fp.write(table.prettify())
            fp.close()
        time.sleep(0.1 + random.random() / 8)
    else:
        with open(f"data/lad_catalog/raw/pages/page_{page}_table.html", "r", encoding="UTF-8") as fp:
            table = BeautifulSoup(fp.read(), features="html.parser")
    print(f"Page {page}/{max_pages}...")
    catalog_table_data.extend(scrape_lad_page_table(table))
raw_catalog_data = {}
total_item_count = 0
if os.path.exists('data/lad_catalog/raw/catalog_item_tables.json'):
    raw_catalog_data = open_json('data/lad_catalog/raw/catalog_item_tables.json')
...
for item in catalog_table_data:
    if item['source'] not in raw_catalog_data:
        ...
        if (total_item_count % 25) == 0:
            ...
            save_json('data/lad_catalog/raw/catalog_item_tables.json', raw_catalog_data)
    total_item_count += 1
...
save_json('data/lad_catalog/catalog.json', catalog_data)